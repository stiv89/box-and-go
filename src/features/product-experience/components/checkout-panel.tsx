...
import { USER_ERROR_QUOTE } from "@/lib/user-facing-error";
...
const DEMO_CART_KEY = "box-and-go-demo-cart";

function createDemoOrderId() {
  return `DEMO-${Date.now().toString(36).slice(-4)}${Math.random().toString(36).slice(2, 4)}`.toUpperCase();
}
...
export function CheckoutPanel({
  ...
  const [purchased, setPurchased] = useState<CheckoutSuccess | null>(null);
  ...
  async function saveToCart() {
    if (incomplete) {
      notify.warning(`This box isn’t complete. Place ${boxSize} chocolates before saving to cart.`);
      return;
    }
    if (!name.trim() || !emailValid) {
      setDraftNote("Add a name and email to save this order to your cart.");
      notify.warning("Add a name and a valid email to save this order.");
      return;
    }
    ...
    try {
      ...
      /* demo cart is still shown even if storage is blocked */
    }
    ...
    await wait(280);
    setPurchased(result);
    notify.success("Saved to cart. This is a simulated checkout.");
  }

  function finishPurchase() {
    if (!purchased || !isPresentableCheckoutSuccess(purchased)) return;
    onSuccess(purchased);
  }
    ...
    <div className="space-y-6 motion-reduce:animate-none motion-reduce:transform-none animate-in fade-in slide-in-from-right-3 duration-300">
      <div>
        ...
        <p className="mt-1.5 max-w-sm text-sm leading-relaxed text-neutral-500">
          Save the order to your cart. Checkout is simulated — nothing is charged.
        </p>
      </div>
      ...
      >
        ...
        <Button type="submit" className="min-h-10 w-full" disabled={!canCheckout}>
        ...
        <p className="text-[11px] text-neutral-500">
          Simulated checkout. The order summary stays on this device. Nothing is charged.
        </p>
        <Button
          type="button"
          variant="ghost"
      ...
      <Dialog
        open={purchased !== null}
        onOpenChange={(open) => {
          if (!open) finishPurchase();
        }}
      >
        <DialogContent
          className="max-w-sm border border-[var(--chocolate-light)]/25 bg-[var(--cream)] sm:max-w-sm"
          showCloseButton={false}
        >
          <DialogHeader className="items-center text-center">
            <span className="flex size-12 items-center justify-center rounded-full bg-[var(--chocolate)] text-white">
            ...
            <DialogDescription>
              Your box is in the cart. This is a simulated checkout — nothing was charged.
            </DialogDescription>
          </DialogHeader>
          {purchased?.source === "demo-cart" ? (
            <dl className="space-y-2 rounded-2xl border border-[var(--chocolate-light)]/30 bg-white/70 p-4 text-sm">
              <div className="flex justify-between gap-3">
                <dt className="text-neutral-500">Order</dt>
                <dd className="font-medium tabular-nums">{purchased.orderId}</dd>
              </div>
              <div className="flex justify-between gap-3">
                <dt className="text-neutral-500">Box</dt>
                <dd className="font-medium">{purchased.boxLabel}</dd>
              </div>
              <div className="flex justify-between gap-3">
                <dt className="text-neutral-500">Quantity</dt>
                <dd className="font-medium tabular-nums">{purchased.quantity}</dd>
              </div>
              <div className="flex justify-between gap-3">
                <dt className="text-neutral-500">Email</dt>
                <dd className="max-w-[60%] truncate text-right font-medium">{purchased.email}</dd>
              </div>
            </dl>
          ) : null}
          <Button type="button" className="min-h-10 w-full" onClick={finishPurchase}>
            Continue
          </Button>
        </DialogContent>
      </Dialog>
